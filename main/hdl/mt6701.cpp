...
MT6701::MT6701(spi_host_device_t spi_host, gpio_num_t sclk_io,
               gpio_num_t miso_io, gpio_num_t mosi_io, gpio_num_t cs_io) {
  _spi_host = spi_host;
  _sclk_io = sclk_io;
  _miso_io = miso_io;
  _mosi_io = mosi_io;
...
float MT6701::getSensorAngle() {
  ...
  if (_spi_host != SPI_HOST_MAX) {
    ...
    ESP_RETURN_ON_FALSE(ret == ESP_OK, 0.0, TAG, "SPI transaction failed: %s",
                        esp_err_to_name(ret));

    uint32_t spi_32 = ((int32_t)spi_transaction.rx_data[0] << 16) |
                      ((int32_t)spi_transaction.rx_data[1] << 8) |
                      spi_transaction.rx_data[2];
    ...
    if (i2c_master_write_read_device(_i2c_port, 0x06, &raw_angle_addr, 1,
                                     raw_angle_buffer, 2,
                                     1000 / portTICK_PERIOD_MS) != ESP_OK) {
      return -1;
    }
    angle = ((int)((raw_angle_buffer[0] << 6) | (raw_angle_buffer[1] >> 2))) *
            0.00038349519f; /*!< Converts raw data into angle information in
                               radians. */
    return angle;
  }

...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SET_PKT_LENGTH_COMPLETE_EVT:
        ESP_LOGI(GATTS_TAG, "packet length updated: rx = %d, tx = %d, status = %d",
                 ...
                 param->pkt_data_length_cmpl.params.tx_len, param->pkt_data_length_cmpl.status);
        break;
    default:
        break;
    }
                                          ...
                                          esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
        ESP_LOGI(GATTS_TAG, "REGISTER_APP_EVT, status %d, app_id %d", param->reg.status,
                 param->reg.app_id);
...
void bluetooch_task(void *param)
{
    uint8_t notify_data[39];
    while (1) {
        if (connect_flag == 1) {
            xQueueReceive((QueueHandle_t)param, &notify_data, portMAX_DELAY);
